      ...
      <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
        ...
        <div id="mobile-menu" className={`mobile-menu ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}>
          <nav aria-label="Mobile navigation">
            {navigation.map(([label, href], index) => <a href={href} key={href} onClick={() => setMenuOpen(false)}><span>0{index + 1}</span>{label}</a>)}
          </nav>
  ...
  return (
    <footer className="footer">
      <div className="footer-grid">
        <div><Logo /><p>Modern websites built with clarity, character and precise execution.</p></div>
      ...
      <div className="footer-word">AHPIXEL</div>
      <div className="footer-bottom"><span>© 2026 AHPixel Studio.</span><span>Web design · Development · Digital experiences</span><span>Lima, Peru · Working worldwide</span></div>
    </footer>
  );
}
