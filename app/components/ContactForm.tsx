...
export function ContactForm({ language = "en" }: { language?: Language }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw Object.assign(new Error(result.message || t.genericError), { status: response.status });
      setSuccessData({ name, websiteType: optionLabel(websiteTypes.find((item) => item.value === websiteType) || websiteTypes[0], language) });
      setStatus("success");
      trackStudioEvent("contact_form_submitted", { language, websiteType, sourcePage: location.pathname });
      form.reset();
    } catch (error) {
      const responseStatus = typeof error === "object" && error && "status" in error ? Number(error.status) : 0;
      setStatus("error");
      setStatusMessage(responseStatus === 429 ? t.rateError : t.genericError);
    } finally {
